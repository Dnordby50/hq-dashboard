...
const {buildSql}=require('./reconcile-historical-completion-dates.cjs');
...
(async()=>{
  const db=new PGlite();
  await db.exec(`create role service_role; create table jobs(id uuid primary key,customer_id uuid,price numeric,completed_date date,dripjobs_deal_id text,source text,archived_at timestamptz,voided_at timestamptz,signed_date date,status text);
  ...
  for(const r of plan)await db.query(`insert into jobs values($1,$2,$3,$4,$5,'native',null,null,'2026-01-01','completed')`,[r.job_id,r.customer_id,r.price,r.old_date,r.external_deal]);
  ...
  assert.deepEqual(after.map(({completed_date,...r})=>r),before.map(({completed_date,...r})=>r));console.log('PASS all non-date fields preserved');
  for(const row of (await db.query('select before_json,after_json from audit_log')).rows)assert.equal(row.after_json.source,row.before_json.source);
  console.log('PASS evidence metadata preserves original audit snapshot source');
  await db.exec(sql);assert.equal((await db.query('select * from audit_log')).rows.length,2);console.log('PASS replay is idempotent');
  await db.exec('delete from audit_log');for(const r of plan)await db.query('update jobs set completed_date=$1 where id=$2',[r.old_date,r.job_id]);
  ...
  assert.equal((await db.query('select * from audit_log')).rows.length,0);assert.equal((await db.query('select completed_date from jobs where id=$1',[id(1)])).rows[0].completed_date,null);console.log('PASS concurrent drift rolls back every change');
