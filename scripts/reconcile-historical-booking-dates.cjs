...
const KEY='historical-booking-dates-2026-09-22';
...
function buildSql(plan,sourceHash,apply=false){
  ...
  const read=`${reviewed} select r.*,j.signed_date current_date,j.price current_price,j.customer_id current_customer,j.source,j.archived_at,j.voided_at,j.dripjobs_deal_id from reviewed r left join public.jobs j on j.id=r.id order by r.id`;
...
begin
  ...
  for r in ${reviewed} select * from reviewed order by id loop
    ...
    if not found or before_row.customer_id is distinct from r.customer_id or before_row.price is distinct from r.price
      ...
      raise exception 'Reviewed booking evidence changed for %',r.id;
    end if;
    if exists(select 1 from public.audit_log where entity_id=r.id and action='historical_booking_date' and after_json->>'reconciliation_key'=${quote(KEY)}) then
      if before_row.signed_date is distinct from r.source_date then raise exception 'Reconciled date changed for %',r.id; end if;
      continue;
    end if;
    if before_row.signed_date is distinct from r.old_date then raise exception 'Booking date changed for %',r.id; end if;
    update public.jobs set signed_date=r.source_date where id=r.id returning * into after_row;
    insert into public.audit_log(action,entity_type,entity_id,before_json,after_json)
    values('historical_booking_date','jobs',r.id,to_jsonb(before_row),to_jsonb(after_row)||jsonb_build_object(
      'reconciliation_key',${quote(KEY)},'source','PEC DripJobs Closed Deals export 2026-09-22',
      'source_sha256',${quote(sourceHash)},'match','Unique customer name plus email or phone and exact contract amount; original accepted date'));
  end loop;
end;
$repair$;
select count(*) as repaired_records from public.audit_log where action='historical_booking_date' and after_json->>'reconciliation_key'=${quote(KEY)};
commit;`;
}
...
if(require.main===module){
  ...
  if(!file||!source)throw new Error('Usage: node scripts/reconcile-historical-booking-dates.cjs PRIVATE_PLAN.json --source=PRIVATE_CLOSED_DEALS.csv [--dry-run|--apply]');
