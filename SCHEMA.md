...
Refreshed 2026-08-12 (Claude Code, prompt 87) after applying two data-only migrations live via MCP: settings keys `estimate_accept_slack_enabled` ('true'; the Slack accept celebration post in pec-public-estimate.cjs notifyOffice, a checkbox inside the Slack card in Settings > Estimates) and `accept_celebration_enabled` ('true'; the staff-dashboard confetti + toast on acceptance, front-of-card in Settings > Estimates, read once per page life by pecStartRevokedLoginWatch). No table or column changed. Live settings count at refresh: 104 rows (the documented 98 had drifted by 4 before these two seeds; the live schema wins).
...
### settings
RLS: enabled · rows: 104 (live count 2026-08-12)
...
Keys added 2026-08-12 (prompt 87): estimate_accept_slack_enabled ('true'; the #epoxysales accept celebration post, independent of estimate_view_slack_enabled; missing row = on) and accept_celebration_enabled ('true'; dashboard confetti + toast on acceptance; missing row = on). Both in Settings > Estimates. Inserted insert-only. Settings 102 rows to 104.
