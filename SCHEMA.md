...
Note: warranty_snapshot (2026-08-16, prompt 93) — `{sections:[{id,title,body,images}],frozen_at}`, the brand's active kind='warranty' presentation sections frozen by markEstimateSent on EVERY send (re-send re-snapshots; no active warranty writes null). The customer page renders the snapshot when present, the live sections while the estimate is still open, and nothing on an accepted estimate with no snapshot (signed before the feature). Written by its own PATCH that never includes status, because trg_estimate_status_guard fails whole updates carrying a stale status.
