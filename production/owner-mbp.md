...
## Editable MBP inputs and PEC refresh (2026-09-08)

The fresh six-sheet MBP export confirms input fields by original formula and input-fill metadata. Brand Sales exposes annual C5:C7 and K4:K6; weekly D/J/V/AB actuals, AH seasonal allocation, and optional AL/AO conversion overrides. Brand Revenue exposes annual C4/C6, weekly D/K actuals, P allocation, and W/X custom plan/actual. Revenue TOTAL exposes only its independent W/X custom metric inputs. Total core values, all cumulative fields, ratios, trends, dates and formula cells stay calculated. Hidden insurance/claims inputs remain outside the existing active Typical scenario. Source protection flags were not usable for editability; explicit input fills and formula presence agreed across the six sheets.

`owner-mbp-inputs.js` owns the field allowlist, typed validation, immutable edit batches and PEC source merging. Keys use `lineId/kind/weekEnding/field`, with `annual` for annual inputs. `body.mbpCellState` tracks manual/topcoat origin, update time and the latest source value/availability. Existing non-null inputs without provenance remain unchanged and do not receive a false yellow marker. Fresh PEC source values fill eligible blanks or refresh TopCoat-tagged fields. Manual zero and null remain explicit overrides. Unavailable/failed/future/stale source data never becomes a confirmed zero or removes an override. FTP is never a live source target. Use TopCoat requires a currently available PEC source and resets that field to automatic. `body.mbpLiveState` stores freshness and source warnings when edits are saved.

`owner-mbp-live.cjs` retains the saved Sunday calendar keys while measuring the preceding Sunday-through-Saturday period in Phoenix time. Working views display Saturday closing dates; the original source view retains Sunday labels. Sources are distinct PEC contacts by earliest nondeleted linked pipeline lead creation date, proposals by first successful email/text send, and core CRM jobs by signed date for booked jobs/dollars, and completed date for produced dollars, with archived/voided jobs excluded. Current-week values are partial; future weeks are omitted. Coverage begins at the first complete week after each source's earliest recorded activity. Duplicated deal IDs, missing prices, missing booking/completion dates, missing coverage and failed source reads disable affected values. The server-only sales-metrics.cjs adapter merges the first-send ledger with exact-token historical delivery receipts, taking the earliest event across all history before applying the Phoenix week. Current proposal status, resends, dual-channel sends and later deletion cannot add another proposal. It never substitutes mutable estimates.sent_at for first-send evidence. New customer records without a pipeline lead, unresolved delivery attempts, historical sent stamps without receipts, source failures and capped pagination keep affected counts unavailable. First sends are recorded by the server; uncertain provider outcomes block another send until verified. Manual/imported values remain editable and explicitly marked as such. Labor hours remain manual because legacy hours have no work date and some imported time remains unassigned. Existing imported amounts retain their original meaning; produced revenue is not cash collected.

The authenticated `mbp-live` read route returns source data without writing owner records. Opening working Sales/Revenue views refreshes the source; configurable polling runs only while a working plan is open, visible and not being edited. `mbp-inputs` accepts only changed field keys, expected revision, request ID and optional plan status/reporting week. It refreshes eligible sources on the server before applying changes. Immutable revision metadata retains a digest of the original request, so dropped-response retries cannot recompute different live data or create a duplicate revision. Old whole-document MBP saves return a reload-required error, retaining drafts instead of discarding provenance. The original source and customer/job records are never modified by plan edits.

`owner-mbp-ui.js` supplies source-aware inputs and a textual manual/source label in addition to color. Saved manual inputs are yellow; blue actual inputs and green planning inputs follow the source distinction. Weekly entry exposes optional conversion overrides without restoring omitted grid columns. Annual inputs edit in place or in Plan assumptions. Seasonal weights must total 100% and can be changed together in the grid or Plan assumptions. Desktop and 390px mobile checks confirmed readable 44px/16px fields and no page-width overflow.

Settings `owner_mbp_live_enabled` (default true) and `owner_mbp_refresh_minutes` (default 5, range 1–60) are owner-scoped configuration, not private data or caches. They are editable in Routine settings. The settings seed is insert-only and does not replace existing configuration. No owner document was bulk rewritten to enable the feature. Tests use synthetic values only.

## Full-screen workbook clone (prompt 108, preview gate)

`owner-mbp-workbook.js` lays the six weekly sheets plus Budget - 2 and Income Statement - 2 out as the source workbook lays them out, and drops TopCoat's existing numbers into the same cells. It calculates nothing: weekly values come from `calculateMbp`, finance values from `calculateFinance` and the saved finance document, and every cell is addressed by the original Excel column letter the engine already uses. Storage, the PEC live refresh and the source rules are unchanged.

`owner-mbp-workbook-layout.js` is generated by `scripts/mbp-workbook-layout.py` from the private .xlsx and holds presentation only: column widths, row heights, merges, hidden rows/columns, outline (group) levels, freeze panes, per-cell fills/fonts/borders/alignment/number-format strings, and the workbook's own static header labels. It contains no values, formulas, account names or owner financial data; `security-publication.test.js` enforces that, because the file is published. Budget - 2 and Income Statement - 2 contribute geometry and styles only and keep their account names in the private finance document. Regenerate with the script; never hand-edit it, and never commit the workbook itself.

Presentation rules reproduced from the source: the red/grey flag columns (SP G, M, S, Y, AF, AS; RP H, N, U) are Grey for a week that has not closed and for a rounded cumulative actual at or above the rounded cumulative plan, Red otherwise, comparing the saved Sunday keys; there is no green state. Excel number formats are parsed by `parseExcelFormat` and applied by `formatExcel`, including accounting alignment, `[Red]` negatives, the accounting dash for zero and `d mmm` week dates. An editable cell shows the workbook's formatted value and swaps to the raw editable number while it has focus. The bottom row totals only the columns the workbook totals; the AJ/AM/AP plan averages the workbook shows are taken over the engine's own plan column for display and are not stored. Quarter blocks, hidden claims and override columns, and hidden control rows follow the workbook. The MBP Support link, the USER INSTRUCTIONS box and the row-1 read-only banner are left out, with their cells kept so nothing shifts. A left-aligned label spills across the empty cells after it and clips at the first one with content, as Excel does; a merged cell never spills.

Editing is Excel-like. Only the cells already in the "Editable MBP inputs" allowlist and the finance input ranges accept typing. Enter and Tab move between editable cells and so skip calculated ones, Escape restores the cell, Delete leaves a blank override that stays distinct from zero. Each change saves after `owner_mbp_autosave_ms` of idle through the existing `mbp-inputs` changed-field save or the existing finance save, keeping the request-ID replay and revision-conflict boundary; one save runs at a time and further typing queues the next. A conflict keeps the typed values, shows Conflict in the toolbar and never overwrites. Right-click (or long-press) a yellow manual override offers the same Use TopCoat reset, with unchanged eligibility. Budget - 2 and Income Statement - 2 render the workbook's row outline groups at their saved collapsed state with Excel-style +/- controls; the open/closed state is a per-viewer localStorage convenience wrapped in try/catch, never owner data. A few source rows are hidden outside every outline group, so no +/- control reaches them; when such a row still holds an entered value the toolbar offers "Show N hidden rows with values", off by default so the opening view matches the workbook. Only entered values count, never a formula cell or an explicit zero, and revealing them does not expand any outline group. Income Statement - 2 rows 73 and 74 are the current case. The Sales Plan and Revenue Produced sheets deliberately have no outline bar (Dylan, 2026-09-25); their hidden claims and override columns stay hidden with no control.

Settings `owner_mbp_autosave_ms` (default 800, range 300-5000) and `owner_mbp_workbook_default_sheet` (default `sales_total`) are owner-scoped configuration in Routine settings, seeded insert-only by `20260925160229_owner_mbp_workbook_settings.sql`. `owner-routine.js` keeps its own copy of the eight sheet ids so the server function does not import the browser layout module; `owner-mbp-workbook.test.js` asserts the two lists match.
