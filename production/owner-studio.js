...
import { mbpInputFields, applyMbpLive, applyMbpEdits, currentMbpWeek, mbpSaturday, mbpSunday } from './owner-mbp-inputs.js';
...
export function renderMbpGrid(sheet, period='all', {body=null,readOnly=false,sourceCalendar=readOnly}={}) {
  ...
  const displayWeek=value=>sourceCalendar?value:mbpSaturday(value);
  const rows=sheet.rows.filter(r=>period==='all'||(period.startsWith('month:')?displayWeek(r.weekEnding).slice(5,7)===period.slice(6):String(r.quarter).replace('Q','')===period.replace('Q','')));
  ...
  return `<div class="tc-mbp-scroll" tabindex="0" role="region" aria-label="${e(sheet.sourceTabName)}. Scroll within the table for weeks and columns. Headers stay visible."><table class="tc-mbp-table"><caption class="tc-sr-only">${e(sheet.sourceTabName)} weekly plan and actuals</caption><thead><tr><th rowspan="3">QTR</th><th rowspan="3" class="tc-mbp-date">WEEK ENDING</th>${groups.map((g,i)=>`<th colspan="${g.cols.length}" data-mbp-group="${i}" class="tc-mbp-divider">${g.label}</th>`).join('')}</tr><tr>${groups.map(g=>g.sub.map(([text,n])=>`<th colspan="${n}">${text}</th>`).join('')).join('')}</tr><tr>${groups.map(g=>g.heads.map(h=>`<th scope="col">${h}</th>`).join('')).join('')}</tr></thead><tbody>${rows.map(r=>`<tr><td>Q${String(r.quarter).replace('Q','')}</td><th scope="row" class="tc-mbp-date">${readOnly||sheet.businessLineId==='total'&&sheet.kind==='sales'?`<span data-week="${r.weekEnding}">${displayWeek(r.weekEnding)}</span>`:`<button type="button" data-action="edit-week" data-week="${r.weekEnding}">${displayWeek(r.weekEnding)}</button>`}</th>${cols.map(c=>cell(r,c)).join('')}</tr>`).join('')}</tbody><tfoot><tr><td></td><td class="tc-mbp-date">Full-year footer</td>${cols.map(c=>cell({v:sheet.footer},c)).join('')}</tr></tfoot></table></div><div class="tc-mbp-foot"><span>${rows.length} of ${sheet.rows.length} weeks. ${sourceCalendar?'Original workbook dates.':'Sunday–Saturday · Arizona time. Imported/manual values retain their original source meaning.'} ${period.startsWith('month:')?'Month uses the week-ending date. ':''}Headers stay visible while scrolling. Summary and footer remain full-year.</span><span>* Missing inputs, not a confirmed zero.</span></div>`;
}

// The source drilldown is kept only in signed-in memory, never saved into the workbook.
export function renderSalesIntegrity(feed,{kind='sales',period='all'}={}){
  if(!feed||feed.disabled)return '';
  const fields=kind==='revenue'?['producedDollars']:['leads','estimates','jobsBooked','bookedDollars'];
  const labels={leads:'Leads',estimates:'Estimates sent',jobsBooked:'Jobs booked',bookedDollars:'Dollars booked',producedDollars:'Dollars produced'};
  const basis={legacy_record_created:'Legacy record date; original inquiry not yet verified',recorded_inquiry:'Recorded original inquiry date',legacy_current_contract:'Legacy current contract price',owner_confirmation:'Owner-confirmed original date',first_send_record:'First successful delivery',email_receipt:'Email receipt',sms_receipt:'Text receipt',booked:'Acceptance amount',completed:'Completion amount',completion_amended:'Audited completion correction',booked_adjusted:'Booked amount adjustment',completed_adjusted:'Completed amount adjustment'};
  const weeks=(feed.weeks||[]).filter(row=>period==='all'||(period.startsWith('month:')?row.end?.slice(5,7)===period.slice(6):Math.ceil(Number(row.end?.slice(5,7))/3)===Number(period.replace('Q',''))));
  const exceptions=(feed.exceptions||[]).filter(row=>fields.includes(row.metric)&&weeks.some(week=>(!row.from||row.from<=week.end)&&(!row.through||row.through>=week.start)));
  const unverified=weeks.some(row=>fields.some(field=>row.available?.[field]!==true));
  return `<details class="tc-mbp-sources" style="overflow-wrap:anywhere"><summary>Reporting health: ${exceptions.length?`${exceptions.length} ${exceptions.length===1?'item needs':'items need'} review`:unverified?'some totals remain unverified':'no open data exceptions'}</summary><p>Automatic records for PEC. Manual and imported workbook values remain separate. An unavailable total is not a confirmed zero.</p>${exceptions.length?`<h3>Audit tasks</h3><ol>${exceptions.map(row=>`<li><strong>${e(row.label)} · ${e(labels[row.metric])}</strong><p>${e(row.reason)} ${e(row.action)}</p><p class="tc-small">Record: ${e(row.recordId||'Source event')} · Affected dates: ${row.from&&row.through?`${e(row.from)} through ${e(row.through)}`:'unknown; historical weeks may be affected'}</p></li>`).join('')}</ol>`:''}<h3>Records behind weekly totals</h3>${weeks.slice().reverse().map(week=>`<details><summary>${e(week.start)} to ${e(week.end)}${week.partial?' · In progress':''}</summary>${fields.map(field=>{const records=week.sources?.[field]||[];return `<details><summary>${e(labels[field])}: ${week.available?.[field]===true?e(fmt(week.actual?.[field],field.includes('Dollars')?'money':'number')):'Unverified'} · ${records.length} source record${records.length===1?'':'s'}</summary>${records.length?`<ul>${records.map(row=>`<li><strong>${e(row.label)}</strong> · ${e(row.date||'Date missing')}${row.amount!==undefined?` · ${row.amount==null?'Amount missing':e(new Intl.NumberFormat('en-US',{style:'currency',currency:'USD'}).format(row.amount))}`:''}<div class="tc-small">${e(basis[row.basis]||row.basis||'Recorded source')}${row.source?` · ${e(row.source)}`:''}${row.evidence?` · Evidence: ${e(row.evidence)}`:''} · Record: ${e(row.id)}</div></li>`).join('')}</ul>`:'<p>No dated records in this week. Check availability and audit tasks before interpreting this as zero.</p>'}</details>`;}).join('')}</details>`).join('')}</details>`;
}
...
export function createOwnerStudio({ getSession, openOwner, onAccess=()=>{}, fetchImpl=fetch, now=()=>new Date() }) {
  ...
  async function refreshMbpLive() {
    ...
    try {
      ...
      if(!mbpSourceFeed||Date.parse(feed.queriedAt)>=Date.parse(mbpSourceFeed.queriedAt))mbpSourceFeed=feed;
  ...
  const save=async(key,body)=>{
    ...
    if(key.startsWith('finance:')) {
      // Refuse locally what the endpoint would refuse, so a too-large year never leaves the screen.
      const tooLarge=financeSaveSizeError(new TextEncoder().encode(JSON.stringify({key,body,revision:doc?.revision||0,requestId:request.id})).length);
  ...
  const incomeCompany=()=>financeCompany??status?.config?.incomeDefaultCompany??'combined';
  const incomeShowEmpty=()=>financeShowEmpty??status?.config?.incomeShowEmpty??false;
  ...
  function mbpLiveNotice(body) {
    ...
    return `<div class="tc-mbp-live"><p>${status.config.mbpLiveEnabled===false?'PEC automatic updates are paused.':`PEC updates on opening these plans and every ${status.config.mbpRefreshMinutes||5} minutes while they are open.`} FTP stays manual. <mark>Yellow fields were manually edited</mark>; PEC overrides stay in place until you choose Use TopCoat.</p>${checked?`<p>TopCoat checked ${e(checked)} Arizona time. The current week is still in progress.</p>`:''}${mbpLiveMessage?`<p role="status">${e(mbpLiveMessage)}</p>`:''}${live?.warnings?.filter(w=>/no pipeline lead|delivery verification|could not be refreshed|no verified|exception\(s\)|source event\(s\)/.test(w)).map(w=>`<p class="tc-warning" role="status">${e(w)}</p>`).join('')||''}${live?.warnings?.length?`<details><summary>Weekly source details</summary>${live.warnings.map(w=>`<p>${e(w)}</p>`).join('')}</details>`:''}</div>`;
  }
  function workbookPage(kind) {
    ...
    return head('YOUR MBP · '+year(),kind==='sales'?'Sales Plan':'Revenue Produced','Edit the original input fields. Calculated totals and cumulative values update when you save.')+
      ...
      `<div class="tc-mbp-jumps"><span>Jump to</span>${mbpGroups(kind).map((g,i)=>button(g.label,'jump',false,`data-group="${i}"`)).join('')}</div>`+renderMbpGrid(sheet,period,{body:doc.body,readOnly:sourceView||editor,sourceCalendar:sourceView})+
  ...
  function financePage() {
    ...
    const tagged=page==='income'&&!financeSource&&!!body?.sheets?.find(s=>s.kind==='income')?.companyRows;
    const controls=`<div class="tc-mbp-bar">${select('Budget year','financeYear',financeYear,financeYears.map(d=>[d.doc_key.slice(8),d.doc_key.slice(8)]),'data-change="finance-year"')}${tagged?select('Company','financeCompany',incomeCompany(),FINANCE_COMPANIES,'data-change="finance-company"'):''}<div class="tc-row">${tagged?button(incomeShowEmpty()?'Hide empty slots':'Show empty slots','finance-show-empty'):''}${button(financeSource?'Working copy':'Original source','finance-source')}${button(financeHidden?'Use source layout':'Show all rows and columns','finance-hidden')}${financeSource?'':button('Add year','finance-new')}</div></div>`;
    ...
    // Company views recalculate a filtered copy; the saved document is never changed by the filter.
  ...
  function collectFinance() {
    ...
    for(const input of mount.querySelectorAll('[data-finance-label]')) {
      ...
      const currentText=current.f?String(computedLabel(row)??''):String(current.v??'');
      if(input.value.trim()===currentText.trim())continue;
      financeSetAccountLabel(body,sheet,row,input.value);
    }
  ...
  function addAccount(sectionId) {
    const body=collectFinance(),sheet=body.sheets.find(s=>s.kind===page),computed=calculateFinance(body).sheets.find(s=>s.id===sheet.id);
    const name=String(mount.querySelector('[name="financeAccountName"]')?.value||'').trim();
    if(!name)throw new Error('Type the new account name first.');
    const slot=financeEmptySlot(sheet,computed,sectionId);
    if(!slot)throw new Error('That section is not available on this income statement.');
    if(slot.row===null)throw new Error(`${slot.section.label} is full (${slot.used} of ${slot.total} slots used). Clear an unused account in that section first; accounts never spill into another section or company.`);
    financeSetAccountLabel(body,sheet,slot.row,name);
    docs.get(financeKey()).body=body;dirty=true;financeAdding=null;
    message=`Added "${name}" to ${slot.section.label} (row ${slot.row}, ${slot.section.company}). Save changes to keep it.`;
  }
  async function switchFinanceCopy(nextYear,nextSource,target) {
    ...
    try {
      ...
      financeYear=String(nextYear);financeSource=nextSource;financeCreate=false;financeCreateRequest=null;financeAdding=null;dirty=false;message='';
    } finally {release();}
  }
  function weeklyEditor(kind,body) {
    const weeks=body.mbp.weekEndings;week=weeks.includes(week)?week:weeks.find(d=>d>=currentMbpWeek(now()))||weeks.at(-1);
  ...
  function draw() {
    ...
    const adding=mount.querySelector('[name="financeAccountName"]');
    if(adding){adding.focus();adding.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();mount.querySelector('[data-action="finance-add-confirm"]')?.click();}});}
    ...
    mount.onchange=async event=>{if(event.target.hasAttribute('data-milestone-toggle'))updateMilestoneProgress(event.target.closest('.tc-milestones'));if(event.target.hasAttribute('data-focus-milestone'))updateMilestoneProgress(event.target.closest('.tc-weekly-rocks'),true);const change=event.target.dataset.change; if(change){
      ...
      if(change==='finance-company'){
        // Keep unsaved entries: fold them into the working copy before re-rendering the other view.
        try{if(!financeSource&&dirty)docs.get(financeKey()).body=collectFinance();}catch(err){event.target.value=incomeCompany();message=err.message;mount.querySelector('.tc-save-status').textContent=message;return;}
        financeCompany=event.target.value;financeAdding=null;draw();return;
      }
      ...
      if(action?.startsWith('finance-')) {
        if(action==='finance-save'||action==='finance-recalculate') {
          ...
          if(action==='finance-save') {
            const release=showOwnerSaving(mount,target,'Saving changes…');
            try{await save(financeKey(),body);financeAdding=null;}
            catch(err){
              // A failed save keeps the screen and its typed values; nothing is redrawn.
              throw new Error(err.status===409?err.message:`Save failed: ${err.message} Your edits are still on this screen and were not saved. Fix the problem and choose Save changes again; do not reload or leave until it saves.`);
            }
            finally{release();}
          }
        ...
        if(action==='finance-show-empty') {if(!financeSource&&dirty)docs.get(financeKey()).body=collectFinance();financeShowEmpty=!incomeShowEmpty();}
        if(action==='finance-add') {if(financeSource)throw new Error('Return to the working copy to add accounts.');if(dirty)docs.get(financeKey()).body=collectFinance();financeAdding=target.dataset.section;}
        if(action==='finance-add-cancel') {if(dirty)docs.get(financeKey()).body=collectFinance();financeAdding=null;}
        if(action==='finance-add-confirm') {if(financeSource)throw new Error('Return to the working copy to add accounts.');addAccount(target.dataset.section);}
