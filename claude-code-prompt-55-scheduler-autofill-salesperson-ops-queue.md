...
3. Both values stay EDITABLE in the schedule modal. The autofill is a prefill, not a lock. A typed override saves exactly as it does today.
3a. **System: live, but a manual pick wins.** The derived value follows the estimate on every render, so a change order or a corrected rep pick shows up on the schedule automatically. But once someone explicitly picks a system in the schedule modal, that pick is an override and stops following the estimate.
...
- Precedence for hours, in order: computed `budgetedHours` > stored `pec_prod_jobs.estimated_hours` > touch-up default (`state.touchupSettings.defaultHours`) for callbacks > blank. State this precedence in a comment at the helper.
...
In `openScheduleModal`, prefill `draft.system_type_id` and `draft.estimated_hours` from the helper, falling back to the current expressions. Both controls stay editable and Save behaves exactly as today (the inline system pick still inserts a default "Main" area when the job has none; the hours field still writes `pec_prod_jobs.estimated_hours`).

**System override semantics (decision 3a).** The derived system is live: it re-resolves on every render, so an upstream change follows through to the schedule. An EXPLICIT pick in the schedule modal is an override and must stick. Implement the override with the mechanism that already exists rather than a new flag: an explicit pick writes the system onto the job's `pec_prod_areas` row (inserting the default "Main" area when there is none), and the helper's precedence already puts a real `pec_prod_areas` system ahead of the CRM-derived one. Verify that ordering holds; if it does not, fix the precedence, do not add a column. Saving the modal WITHOUT touching the system dropdown must not write anything, or every Save would silently freeze the derived value into an override.
...
- Pick a system explicitly in the schedule modal, then change the estimate's system upstream: the schedule keeps the picked one (decision 3a). Then Save the modal on a different job WITHOUT touching its system dropdown, change that job's estimate upstream, and confirm the schedule follows the estimate (the Save did not silently freeze it).
