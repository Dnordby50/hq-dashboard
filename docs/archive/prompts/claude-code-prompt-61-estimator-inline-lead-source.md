...
**LOCKED: the standalone `/estimator/` page keeps working unchanged, and offline is treated as REQUIRED.** Asked five times, Dylan answered "rarely but it has saved us", then "on-site but essentially always online", then "yes, regularly on-site". Take the conservative reading: the offline outbox and service worker are load-bearing, the standalone `/estimator/` stays fully usable, and no part of this build may degrade an offline path to simplify an online one. The cost of being wrong in this direction is a little extra code; the cost of being wrong the other way is a rep losing an estimate in a garage with no signal. `embed=0` behavior (Back button, Dashboard links, outbox) is untouched.
