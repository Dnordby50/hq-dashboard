...
12. Every major feature ships with a settings surface. Its key parameters (on/off, timing, limits, thresholds, quiet hours) must live in the `settings` table and be tunable with no code change. VISIBILITY is rationed: at most TWO controls per feature sit front-of-card (the ones tuned in normal operation); every other parameter goes behind that card's collapsed "Advanced" disclosure. State and caches are NOT settings and must never get a control (or, ideally, a `settings` row) -- if a value is written by the app rather than by a human, it does not belong here. (Hardwired by Dylan, 2026-07-21. Amended 2026-08-08 after an audit found 16 of 78 UI-exposed knobs had never been saved once, and a dozen live parameters had no surface at all: the original wording grew the Settings page monotonically while still missing real gaps. The no-hardcoding guarantee is retained in full; only the must-be-visible clause was relaxed.)
