...
- Idle JWT wedges supabase-js, and only a reload clears it. After the tab sits idle on a stale JWT (diagnosed live: ~57 min), supabase-js's auth-refresh queue wedges: the FIRST `.from(...)` / `auth.*` call afterward hangs forever with ZERO requests on the wire (the fetch never leaves the client), no 401/403. Crucially `supabase.auth.refreshSession()` ALSO hangs (it queues behind the same stuck refresh), so the in-app retry and `withFreshSession`'s refresh-and-retry are both unreliable in this state. A full `location.reload()` re-instantiates the client and is the only reliable fix. Existing mitigations, keep them coherent if you touch this: the no-op auth lock (index.html ~4955), `withFreshSession` 10s timeout/refresh/retry for READS (~5084), `withDeadline` (no-retry deadline) for non-idempotent WRITES like payments (~5106), the visibilitychange idle-probe that prompts a reload after long idle (~5113), the 15s render fence whose Retry button now reloads (~5447). Never wrap a non-idempotent write (payment insert, etc.) in an auto-retrying helper: a retried insert can double-record if the first request actually landed.
